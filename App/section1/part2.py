...
class functions:
	...
	def do_negativeMultiplication(line):
		...
		12*200 == 12*20*100'''
		number1 = random.randint(-990,9990)
		number2 = random.randint(-990,-1)
	...
	def do_negativeSubtraction(line):
		'''simple subtraction'''
		numbers = random.randint(-9999,9999),random.randint(-9999,-1)
	...
	def do_negativeBidmas(line):
		...
		while count != length:	
			...
			if op != '(':
				nums=random.randint(1,100),random.randint(-100,-1)	
				if op == '-':
